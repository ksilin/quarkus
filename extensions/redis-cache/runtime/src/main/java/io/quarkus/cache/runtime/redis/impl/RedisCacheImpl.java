package io.quarkus.cache.runtime.redis.impl;
...
import io.quarkus.cache.runtime.redis.RedisCache;
import io.quarkus.cache.runtime.redis.RedisCacheInfo;
...
public class RedisCacheImpl<K, V> extends AbstractCache implements RedisCache {
    ...
    public RedisCacheImpl(RedisCacheInfo cacheInfo) {
        this(cacheInfo,
                // TODO make sure it's added - if redis cache is present, the default redis must be enabled
                Arc.container().select(Redis.class, RedisClientName.Literal.of(cacheInfo.name)).get());
    }
    ...
    public RedisCacheImpl(RedisCacheInfo cacheInfo, Redis redis) {
        ...
        try {
            this.classOfKey = (Class<K>) RedisCacheImpl.class.getClassLoader().loadClass(this.cacheInfo.keyType);
        ...
        if (this.cacheInfo.valueType != null) {
            try {
                this.classOfValue = (Class<V>) RedisCacheImpl.class.getClassLoader().loadClass(this.cacheInfo.valueType);
            } catch (ClassNotFoundException e) {
    ...
    private Uni<Void> set(RedisConnection connection, byte[] key, byte[] value) {
        return connection.send(Request.cmd(Command.SET).arg(key).arg(value).arg("EX").arg(cacheInfo.ttl.toSeconds()))
                .replaceWithVoid();
    }
